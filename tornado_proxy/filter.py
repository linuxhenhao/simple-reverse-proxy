...
filter_regexs={'https?://g\.sci-hub\.tk.*/scholar':'filt_scholar',
        'https?://sci\.ald-lab\.tk':'filt_scihub'}

class Myfilter:
    def __init__(self,filter_regexs):
        self.filer_regexs=filter_regexs
    ...
    def filt_content(self,url,response,**kwargs):
        for rule in self.rules:
            if(rule.match(url)!=None): #in filter rules
                ...
                getattr(self,filt_name)(response,**kwargs)


    def filt_scholar(response,**kwargs): #scholar's filter
            scihub_host=kwargs['scihub_host']
...
#generate new tag to add after more
    ...
    def filt_scihub(response,**kwargs):
        None
